...
onEvent('item.registry', (event) => {
    ...
    mechanism('Kinetic', '动能构件')
    event.create('makeshift_kinetic_mechanism').displayName('简易动能构件').tooltip("如果没有门瑞欧，构件就不够坚固，无法制作更多的构件。")
    mechanism('Sealed', '橡胶构件')
    mechanism('Sturdy', '坚固构件')
    mechanism('Plastic', '塑料构件', RARITY_UNCOMMON)
    mechanism('Integrational', '魔源构件', RARITY_UNCOMMON)
    mechanism('Radiant', '光辉构件', RARITY_UNCOMMON)
    mechanism('Time', '时间构件', RARITY_RARE)
    mechanism('Supercritical', '超临界构件', RARITY_RARE)
    tool('Saw', '锯子')
    seed('Quartz', '石英')
