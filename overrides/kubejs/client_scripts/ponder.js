      ...
      .scene(
        ...
        (scene, util) => {
            ...
            scene.text(50, "This is the Source Time Crystal! ", [2.5, 3.5, 2.5])
              ...
              .attachKeyFrame();
            scene.idle(50)
            scene.text(35, "Now ensues a Text Driven Explanation on how to do it (cause animating it is seriously too hard)!", [2.5, 3.5, 2.5])
            scene.idle(45)
            scene.world.setBlock([sx+1, sy, sz], "ars_nouveau:creative_source_jar", true);
            scene.text(35, "But before that. These are Spell Turrets, they will be used to automate the Time Crystal.", [sx, sy, sz]).attachKeyFrame()
            scene.idle(45)
            ...
            scene.idle(50)
            scene.text(50, "To get back to the original Point though, heres how the Time Crystal works. First Ill explain it simply, and then afterwards with Code for the nerds.",[sx, sy+2, sz])
            .attachKeyFrame()
            scene.idle(60)
            scene.text(45, "The Time Crystal Spawns with a set of Spells it Desires",[sx, sy+2, sz])
            scene.idle(60)
            scene.text(45, `These Spells can only be in the range of these spells: "hex","light","gravity","ignite","cold_snap", "intangible","freeze","crush","grow","firework"  `,[sx, sy+2, sz]).attachKeyFrame()
            scene.idle(60)
            scene.text(45, "Everytime You Shoot a Spell the Time Crystal, it checks if The Spell Contains one of the Desired Spells, and then Ticks that off its internal list. Are all spells completed, it will convert to ancient debris!",[sx, sy+2, sz])
            scene.idle(60)
            scene.text(50, "The Easiest way to Cheese the Time Crystal would just be to shoot all of the spells in the list at it, keep in mind though that The Crystal has a small cooldown, before it can accept more spells. ",[sx, sy+2, sz]).attachKeyFrame()
            scene.idle(60)
            scene.text(100, 'Now for the Nerdier Explanation: The Time Crystal takes 5 random entries out of this array: "hex","light","gravity","ignite","cold_snap", "intangible","freeze","crush","grow","firework". The 5 Spells are Defined by the following. It oversimplified does a 50/50 Split on the Possible Spell array, and then makes that its internal Spell Goal array. Each spell, no matter the correct matching internal spells will only increment the Time Crystals Stage by 1. Thats basically it. Dont overstress it, just fire spells at it :) ',[sx, sy+2, sz]).attachKeyFrame()
        }
      )
      event
